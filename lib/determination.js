...
module.exports = {
  ...
  getQuorum(list, blockHash, txid, full = false) {
    // if txid includes "-" then we know it's a MN collateral outpoint (vin) -
    // maybe better seperate entry points?
    if (txid.includes('-')) {
  ...
  getQuorums(list, quorumCalculationHash, full = false) {
...
var GetAllQuorums = function (mnList, calcHash, full) {
  ...
  const sortedMnList = mnList.sort((m1, m2) => (m1.vin > m2.vin ? 1 : m2.vin > m1.vin ? -1 : 0));
 ...
 * to make the shuffling deterministic and repeatable based on a seed which in our case is the QuorumCalculationHash
 */
