...
module.exports = {
    ...
    getQuorums: function(quorumCalculationHash, list) {
        if(!list) {
            //get our own server side list: list = dashcore.getMasternodesList()
        }
...
var GetAllQuorums = function(calcHash, mnList) {
    ...
    let sortedMnList = mnList.sort((m1, m2) => {
        return (getDelta(m1.vin, calcHash) > getDelta(m2.vin, calcHash)) ? 1 :
            ((getDelta(m2.vin, calcHash) > getDelta(m1.vin, calcHash)) ? -1 : 0);
    });

    return sortedMnList;
};

var GetQuorumForDapi = function(mnList, blockHash, vin) {

    //XOR blockhash and user regtx to get random position on search space influenced by lastblockhash and user
    let refHash = GetBitwiseXOR(GetTruelyRandomBlockHash(blockHash), vin);
