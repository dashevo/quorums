...
const getQuorumForUser = (mnList, calcHash, txid, full) => {
  ...
  quorums.forEach((quorum) => {
    ...
    const distance2 = bigInt(quorum.hash, 16)
      .minus(bigInt(txid, 16))
      .abs();
...
const getQuorum = (list, blockHash, txid, full = false) => {
  if (!list) {
...
const getQuorums = (list, quorumCalculationHash, full = false) => {
