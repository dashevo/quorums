...
const resolveVinFromIp = (mnList) => {
  if (mnList.proRegTxHash) {
    mnList.vin = mnList.proRegTxHash;
  }
  if (mnList.service) {
    mnList.ip = mnList.service;
  }
...
const getAverageHash = (array) => {
  ...
  array.forEach((element) => {
    ...
    if (element.hash) {
      ...
      hash = element.proRegTxHash.substring(0, element.proRegTxHash.indexOf('-'));
    } else if (element.vin) {
      hash = element.vin.substring(0, element.vin.indexOf('-'));
    } else {
